...
| 4 | Which product category is most profitable relative to sales? | Technology (17.4% margin) — the leading category on both margin and revenue |
...
Full queries with their results: [`analysis_queries.sql`](analysis_queries.sql)

---

## Business Insights

Each insight below is backed by queries in [`analysis_queries.sql`](analysis_queries.sql), cited inline so the numbers can be checked against the data.

### 1. Central region's weak margin is a discounting problem, not a demand problem

*Evidence: Q7 (discount and margin by region), Q8 (where in Central the discounting sits).*
...
*Evidence: Q9 (discount depth by sub-category), Q10 (discount–profit correlation), Q11 (loss-making vs profitable orders), Q12 (share discounted past 30%).*
...
*Evidence: Q13 (category margin against category discount).*
...
*Evidence: Q14 (segment revenue and margin).*
